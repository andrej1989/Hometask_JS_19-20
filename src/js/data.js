...
$(function () {
  ...
  for(var i = 0; i < dataLength; i++){
    ...
    parseData[i] = JSON.parse(str);
  };

  console.log(parseData);

  var skills, friends, friend = [], names, name = [];

  // SKILLS
  _.each(parseData, function (item) {
    skills = _.union(item.skills, skills);
  });

  skills = _.sortBy(skills);

  _.each(parseData, function (item) {
    friends = _.union(item.friends, friends);
  });

  //FRIENDS
  _.each(friends, function (item) {
      friend.push(item.name);
  });

  //NAMES
   names = _.sortBy(parseData, function(i){
    return i.friends.length;
   });
